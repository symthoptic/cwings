...
func main() {
	...
	if sshTimeout > 0 {
		...
		color.Yellow("Cwings server configured with timeouts:")
		color.Yellow("  - Maximum connection duration: %s", sshTimeout)
		color.Yellow("  - Idle timeout: %s", sshTimeout)
	}

	color.Blue("Starting Cwings server on port %s...", port)
	...
	for scanner.Scan() {
		...
		if strings.TrimSpace(line) == "q" {
			color.Yellow("Exit command detected. Closing the Cwings server.")
			os.Exit(0)
		}
	}
}
